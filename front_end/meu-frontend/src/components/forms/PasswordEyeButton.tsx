import React from 'react';
import styled from 'styled-components';

// ============================================
// STYLED COMPONENTS
// ============================================

const EyeButton = styled.button.attrs({ type: 'button' })`
  width: 36px;
  height: 36px;
  border-radius: 0.6rem;
  background: transparent;
  border: 1px solid var(--pl-line);
  cursor: pointer;
  display: flex;
  align-items: center;
  justify-content: center;
  color: var(--pl-muted);
  transition: all 0.15s ease;

  &:hover {
    background-color: var(--pl-blue-soft);
    color: var(--pl-blue);
    border-color: var(--pl-blue-soft);
  }
`;

// ============================================
// COMPONENTE PRINCIPAL
// ============================================

interface PasswordEyeButtonProps {
  visualizando: boolean;
  onClick: () => void;
}

export const PasswordEyeButton: React.FC<PasswordEyeButtonProps> = ({ visualizando, onClick }) =>
{
  return (
    <EyeButton onClick={onClick} aria-label={visualizando ? 'Ocultar senha' : 'Mostrar senha'}>
      {visualizando ? (
        <svg
          width="18"
          height="18"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        >
          <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path>
          <line x1="1" y1="1" x2="23" y2="23"></line>
        </svg>
      ) : (
        <svg
          width="18"
          height="18"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        >
          <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
          <circle cx="12" cy="12" r="3"></circle>
        </svg>
      )}
    </EyeButton>
  );
};

export default PasswordEyeButton;
