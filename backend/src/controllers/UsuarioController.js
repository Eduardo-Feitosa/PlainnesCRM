...
// Tenta até 10 vezes gerar um número aleatório de 5 dígitos que não exista no banco
const gerarCodigoUnico = async () => {
  const MAX_TENTATIVAS = 10;

  for (let tentativa = 1; tentativa <= MAX_TENTATIVAS; tentativa++) {
    // Gera número aleatório entre 10000 e 99999 (5 dígitos)
    const codigo = Math.floor(10000 + Math.random() * 90000);

    const codigoDisponivel = await modeloUsuario.codigoJaExiste(codigo);
    if (!codigoDisponivel) {
      return codigo;
    }
  }

  // Se após 10 tentativas não encontrou código disponível, lança erro
...
const esquemaCadastro = Joi.object
  ({
    ...
    nomeUser: Joi.string().max(15).required().messages({
      'string.max': 'Nome de usuário deve ter no máximo 15 caracteres',
    ...
    tipoPerfil: Joi.string().valid('publico', 'privado').required().messages
      ({
        'any.only': 'Tipo de perfil deve ser publico ou privado',
        'any.required': 'Tipo de perfil é obrigatório',
      }),
...
const cadastrar = async (req, res) => {
  try {
    ...
    let { nome, email, senha, tipo, tipoPerfil, telefone, setor, cpf, dataNascimento, funcao, cnpj, nomeUser } = req.body;
    ...
    if (await modeloUsuario.nomeUserJaExiste(nomeUser)) {
      return res.status(400).json({ erro: 'voce nao pode escolher esse nome, realize o cadastro novamente' });
    }
    ...
    // Passo 4: Gerar código único de 5 dígitos para o usuário
    // (lança erro se não conseguir após 10 tentativas — cai no catch abaixo)
    const codigoGerado = await gerarCodigoUnico();

    // Passo 5: Chamar o model para criar o usuário
    const id = await modeloUsuario.criar
      ({
        nome, email, senha, tipo, tipoPerfil, telefone, setor, cpf, dataNascimento, funcao, cnpj, codigo: codigoGerado, nomeUser
      });
...
const atualizarDados = async (req, res) => {
  try {
    ...
    let { nome, email, tipo, tipoPerfil, telefone, setor, cpf, dataNascimento, funcao, cnpj } = req.body;
    ...
    const atualizou = await modeloUsuario.atualizarDados(idUsuarioLogado,
      {
        nome, email, tipo, tipoPerfil, telefone, setor, cpf, dataNascimento, funcao, cnpj
      });
    ...
    res.json
      ({
        ...
        usuario: {
          ...
          tipoPerfil: usuarioAtualizado.tipoPerfil || null,
          codigo: usuarioAtualizado.codigo || null,
        }
      });
